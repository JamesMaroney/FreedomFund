...
import { playCoinSound, playTadaSound, playSpringSound, primeAudio } from '../utils/audio'
...
const AUDIO_OPTIONS: { value: AudioClip; label: string; emoji: string; description: string }[] = [
  { value: 'coins',  label: 'Sonic Rings',   emoji: '🪙', description: 'Scatter of coin pings' },
  { value: 'tada',   label: 'Ta-da Fanfare', emoji: '🎺', description: 'Brass triumphant chord' },
  { value: 'spring', label: 'Spring Boing',  emoji: '🎵', description: 'Bouncy spring tone' },
  { value: 'random', label: 'Random',        emoji: '🎲', description: 'Surprise me each time' },
]
...
function PresetItem({
  preset, isEditing, isDirty, emojiPickerId,
  onToggleEdit, onUpdate, onSave, onRemove, onToggleEmojiPicker,
}: PresetItemProps) {
  ...
  return (
    <Reorder.Item
      ...
      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', position: 'relative', overflow: 'visible' }}
    >
      ...
      <button className="settings-preset-summary" onClick={() => onToggleEdit(preset.id)}>
        <span className="settings-preset-emoji">{preset.emoji}</span>
        <span className="settings-preset-name">{preset.label}</span>
        <span className="settings-preset-amount">{formatCents(preset.amount)}</span>
            ...
            <div className="settings-preset-form-inner">
              ...
              <div className="settings-preset-row-top">
                ...
                <div className="settings-preset-field">
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    >
                      {preset.emoji} <span style={{ fontSize: '0.65rem', opacity: 0.5 }}>▼</span>
                    </button>
                    <AnimatePresence>
                        ...
                        >
                          {EMOJI_SUGGESTIONS.map((em) => (
                            <button
                              ...
                              onClick={() => { onUpdate(preset.id, { emoji: em }); onToggleEmojiPicker('') }}
                            >
                              {em}
                            </button>
                          ))}
              ...
              <div className="settings-preset-field">
                ...
                <input
                  ...
                  maxLength={20}
                  onBlur={(e) => onUpdate(preset.id, { label: e.target.value || preset.label })}
                  onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur() }}
                />
              </div>
              ...
              {/* Description */}
              <div className="settings-preset-field">
                <label className="settings-preset-field-label">Description</label>
                <input
                  className="settings-preset-text-input"
                  defaultValue={preset.description}
                  maxLength={40}
                  placeholder="e.g. Packed lunch instead"
                  onBlur={(e) => onUpdate(preset.id, { description: e.target.value })}
                  onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur() }}
                />
              </div>
              ...
              <div className="settings-preset-actions">
                {isDirty && (
                  <button className="settings-preset-save-btn" onClick={() => onSave(preset.id)}>
                    Save
                  </button>
                )}
                <button className="settings-preset-delete-btn" onClick={() => onRemove(preset.id)}>
                  Remove chip
                </button>
              </div>
            </div>
...
function TipPresetsSection({ presets, onChange }: { presets: TipPreset[]; onChange: (p: TipPreset[]) => void }) {
  ...
  const add = () => {
    ...
    const newPreset: TipPreset = { id: generateId(), amount: 500, label: 'New', emoji: '💰', description: 'New preset' }
  ...
  return (
    <div className="settings-section">
      <span className="settings-section-label">TIP CHIPS ({presets.length}/{MAX_PRESETS})</span>
      <Reorder.Group
        axis="y"
        values={presets}
        onReorder={onChange}
...
export default function SettingsPanel({
  isOpen, onClose, audioClip, onAudioClipChange,
  goals, onGoalsChange, tipPresets, onTipPresetsChange, onReset,
}: Props) {
        ...
        <>
          ...
          >
            ...
            <div className="settings-body">
              ...
              {/* ── Celebration Sound ── */}
              <div className="settings-section">
                <span className="settings-section-label">CELEBRATION SOUND</span>
                  ...
                  {AUDIO_OPTIONS.map((opt) => (
                    ...
                    >
                      <div className="settings-audio-btn-left">
                        <span className="settings-audio-emoji">{opt.emoji}</span>
                        <div className="settings-audio-text">
                          <span className="settings-audio-label">{opt.label}</span>
                          <span className="settings-audio-desc">{opt.description}</span>
                        </div>
                      </div>
                      <div className="settings-audio-btn-right">
                        {audioClip === opt.value && <span className="settings-audio-check">✓</span>}
              ...
              {/* ── Tip Chips ── */}
              <TipPresetsSection presets={tipPresets} onChange={onTipPresetsChange} />
              ...
              <div className="settings-section">
                ...
                <div className="settings-placeholder-card">
                  ...
                  <span className="settings-placeholder-text">Bank connection coming soon</span>
                </div>
              </div>
              ...
              <div className="settings-section settings-danger-section">
                ...
                {!confirmingReset ? (
                  <button className="settings-reset-btn" onClick={() => setConfirmingReset(true)}>
                    Reset unsent balance
                  </button>
                ) : (
                  <div className="settings-reset-confirm">
                    <p className="settings-reset-warning">
                      This will zero out all deposits not yet sent to your bank. This can't be undone.
                    </p>
                    <div className="settings-reset-actions">
                      <button className="settings-reset-confirm-btn" onClick={() => { setConfirmingReset(false); onReset(); onClose() }}>
                        Yes, reset
                      </button>
                      <button className="settings-reset-cancel-btn" onClick={() => setConfirmingReset(false)}>
                        Cancel
                      </button>
                    </div>
                  </div>
