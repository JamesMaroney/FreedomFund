  ...
  return (
    ...
    >
      ...
      <div className="history-right">
        ...
        {deposit.transferred && (
          <span className="history-transferred" title="Transferred">
            ✓
          </span>
        )}
      </div>
...
export default function DepositHistory({ deposits, landscape }: Props) {
  ...
  // ── Landscape: simple static panel, no spring/drag needed ──────────────────
  if (landscape) {
    return (
      <div className="deposit-history deposit-history--landscape">
  ...
  const height = useMotionValue(
    typeof window !== "undefined" ? collapsedH() : 0,
  );
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (
      isOpen.current &&
      (e.target as HTMLElement).closest(".history-list-wrap")
    )
      return;
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (dt > 0)
      lastVY.current =
        ((e.clientY - (startY.current + (startH.current - height.get()))) /
          dt) *
        -1;
    height.set(newH);
    lastT.current = e.timeStamp;
