...
const RING_TRACKS  = ['rgba(255,55,95,0.15)', 'rgba(245,200,66,0.12)', 'rgba(0,214,143,0.12)']

// ─── Component ───────────────────────────────────────────────────────────────

const PANEL_COUNT = 2

export default function HomeScreen({ fundState, onTipYourself, onOpenSettings, installPrompt, onInstall }: Props) {
  ...
  const snapToPage = (target: number) => {
    ...
    animate(x, -target * w, { type: 'spring', stiffness: 380, damping: 36, mass: 0.8 })
    setPage(target)
  }
  ...
  const handleDragEnd = (_: unknown, info: { offset: { x: number }; velocity: { x: number } }) => {
    ...
    let target = page
    if (offset.x < -40 || velocity.x < -300) target = Math.min(page + 1, PANEL_COUNT - 1)
  ...
  return (
    ...
    >
      ...
      <div className="home-header">
        <button className="hamburger-btn" onClick={onOpenSettings} aria-label="Open settings">
        ...
        {installPrompt ? (
          <button className="install-btn" onClick={onInstall}>Add to Home Screen</button>
        ) : (
          <div style={{ width: 36 }} />
        )}
      </div>
      ...
      <div className="rings-swipe-area" ref={containerRef}>
              ...
              <div className="rings-centre">
                ...
                {displayStreak > 0 && (
                  <motion.div
                    ...
                    animate={depositedToday
                      ? { scale: [1, 1.1, 1], transition: { repeat: Infinity, duration: 2.2 } }
                      : {}}
                  >
                    <span className="rings-streak-fire">🔥</span>
                    <span className="rings-streak-count">{displayStreak}</span>
                  </motion.div>
                )}
              ...
              return (
                <div key={i} className="legend-row">
                  <div className="legend-row-left">
                    <span className="legend-dot" style={{ background: r.color, boxShadow: `0 0 6px ${r.color}` }} />
        ...
        <div className="rings-pips">
          {Array.from({ length: PANEL_COUNT }, (_, i) => (
            <span key={i} className={`rings-pip${page === i ? ' active' : ''}`} />
          ))}
        </div>
      </div>

      {/* ── History — anchored at bottom ── */}
