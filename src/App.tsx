...
import type { FreedomFundState, AppUIState, AppAction, Deposit, AudioClip, TipPreset, Goals } from './types'
import { MILESTONE_AMOUNTS, DEFAULT_WEEKLY_GOAL_CENTS, DEFAULT_TIP_PRESETS, DEFAULT_GOALS } from './constants/presets'
...
function uiReducer(state: AppUIState, action: AppAction): AppUIState {
  switch (action.type) {
    ...
    case 'CONFIRM_DEPOSIT':
      return { ...state, screen: 'CELEBRATING', pendingAmount: action.amount, pendingLabel: action.label }
    case 'CELEBRATION_COMPLETE':
      return { ...state, screen: 'TRANSFER_PROMPT' }
    case 'MARK_TRANSFERRED':
    case 'DISMISS':
      return { screen: 'IDLE', pendingAmount: 0, pendingLabel: '', lastDepositId: null }
    default:
      return state
  }
}
...
export default function App() {
  const [fundState, setFundState] = useLocalStorage<FreedomFundState>('freedom-fund-state', DEFAULT_FUND_STATE)
  ...
  const [audioClip, setAudioClip] = useLocalStorage<AudioClip>('freedom-fund-audio-clip', 'coins')
  ...
  const handleSelectAmount = useCallback((amount: number, label: string) => {
    const today = getTodayString()
    const newStreak = calculateNewStreak(fundState.currentStreak, fundState.lastDepositDate)
    ...
    setFundState((prev) => ({
      ...
      milestones: newMilestone ? [...prev.milestones, newMilestone * 100] : prev.milestones,
    }))
